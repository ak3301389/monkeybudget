...
class BackupService {
  ...
  try {
    ...
    final categories = [
      ...(parsed['expense_categories'] as List? ?? []).map((c) => c['name'] as String).toList(),
      ...(parsed['income_categories'] as List? ?? []).map((c) => c['name'] as String).toList(),
    ];
    
    final syncCode = _generateSyncCode();
    
    await _firestore.collection('users').doc(_userId).collection('data').doc('backup').set({
      'syncCode': syncCode,
...
String _generateSyncCode() {
  ...
  try {
    final doc = await _firestore.collection('users').doc(_userId).collection('data').doc('backup').get();
    ...
    if (doc.exists) {
      ...
      try {
        ...
        if (accounts.isNotEmpty) {
          final list = accounts.where((a) => a is Map).map((a) => models.Account.fromJson(Map<String, dynamic>.from(a))).toList();
      ...
      try {
        ...
        if (transactions.isNotEmpty) {
          final list = transactions.where((t) => t is Map).map((t) => models.Transaction.fromJson(Map<String, dynamic>.from(t))).toList();
      ...
      // Categories
      try {
        final categories = (data['categories'] as List? ?? []).where((c) => c is String).map((c) => c.toString()).toList();
        debugPrint('Categories count: ${categories.length}');
        if (categories.isNotEmpty) {
          final expense = categories.where((c) => !['Зарплата', 'Подработка', 'Дежурства', 'Подарок'].contains(c)).toList();
          final income = categories.where((c) => ['Зарплата', 'Подработка', 'Дежурства', 'Подарок'].contains(c)).toList();

          if (expense.isNotEmpty) {
            storageService.saveExpenseCategories(expense.map((c) => models.Category(c, Icons.category, '#607D8B')).toList());
          }
          if (income.isNotEmpty) {
            storageService.saveIncomeCategories(income.map((c) => models.Category(c, Icons.work, '#4CAF50')).toList());
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (subcategories.isNotEmpty) {
          final list = subcategories.where((s) => s is Map).map((s) => models.SubCategory.fromJson(Map<String, dynamic>.from(s))).toList();
      ...
      try {
        ...
        if (payments.isNotEmpty) {
          final list = payments.where((p) => p is Map).map((p) => models.Payment.fromJson(Map<String, dynamic>.from(p))).toList();
